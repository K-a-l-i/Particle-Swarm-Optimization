...
def Soft_M(func, swarmsize=-1, omega=0.7, phip=1.4, phig=1.4, maxiter=2000, minstep=1e-12, minfunc=1e-12, debug=False, processes=1,particle_output=False):
    ...
    i_update = np.logical_and((fx < fp),fxmag <= fpmag)
...
#    print('Stopping search: maximum iterations reached --> {:}'.format(maxiter))
    if feasible(g):
        return fg, count(g), gmag,e,E,int(feasible(g))
    else:
        return fg, count(g), gmag,e,E,int(feasible(g))
