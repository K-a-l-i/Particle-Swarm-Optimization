...
def ce1_f12(x):
    D=np.size(x)
    return 10*D+ np.sum(x[i]**2-10*np.cos(2*np.pi*x[i]) for i in range(D))-1
...
def ce1_f24(x):
   D=np.size(x)
   return  -(1+np.sum(np.sin(10*np.sqrt(np.abs(x[i]))) for i in range(D)))+1
...
con=([c1_f1],False,False,[c1_f22],False)
coneq=(False,False,[ce1_f12],False,[ce1_f24])
